	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'assessments' => array('Assessments', '', './assets/img/assessment.png', 'None'),
			'courses' => array('Courses', '', './assets/img/course.png', 'None'),
			'institutes' => array('Institutes', '', './assets/img/institute.png', 'None'),
			'modules' => array('Modules', '', './assets/img/modules.png', 'None'),
			'questions' => array('Questions', '', './assets/img/questions.png', 'None'),
			'students' => array('Students', '', './assets/img/student.png', 'None'),
			'subjects' => array('Subject', '', './assets/img/subjects.png', 'None'),
			'teachers' => array('Teachers', '', './assets/img/profile.png', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(
			'assessments' => "`assessments`.`assessmentId` as 'assessmentId', `assessments`.`assessmentName` as 'assessmentName', IF(    CHAR_LENGTH(`institutes1`.`instituteCode`) || CHAR_LENGTH(`institutes1`.`instituteName`), CONCAT_WS('',   `institutes1`.`instituteCode`, '-', `institutes1`.`instituteName`), '') as 'instituteNumber', IF(    CHAR_LENGTH(`courses1`.`courseCode`) || CHAR_LENGTH(`courses1`.`courseName`), CONCAT_WS('',   `courses1`.`courseCode`, '-', `courses1`.`courseName`), '') as 'courseId'",
			'courses' => "`courses`.`courseId` as 'courseId', IF(    CHAR_LENGTH(`institutes1`.`instituteCode`) || CHAR_LENGTH(`institutes1`.`instituteName`), CONCAT_WS('',   `institutes1`.`instituteCode`, '-', `institutes1`.`instituteName`), '') as 'instituteNumber', `courses`.`courseName` as 'courseName', `courses`.`courseCode` as 'courseCode', `courses`.`link` as 'link', IF(    CHAR_LENGTH(`teachers1`.`firstname`), CONCAT_WS('',   `teachers1`.`firstname`), '') as 'teacher', IF(    CHAR_LENGTH(`subjects1`.`subjectName`), CONCAT_WS('',   `subjects1`.`subjectName`), '') as 'subjects', `courses`.`description` as 'description', `courses`.`file` as 'file', FORMAT(`courses`.`amount`, 0) as 'amount', if(`courses`.`dateUploaded`,date_format(`courses`.`dateUploaded`,'%d/%m/%Y'),'') as 'dateUploaded', `courses`.`isActivate` as 'isActivate', `courses`.`isApproved` as 'isApproved'",
			'institutes' => "`institutes`.`instituteNumber` as 'instituteNumber', `institutes`.`instituteName` as 'instituteName', `institutes`.`instituteCode` as 'instituteCode', CONCAT_WS('-', LEFT(`institutes`.`phone`,3), MID(`institutes`.`phone`,4,3), RIGHT(`institutes`.`phone`,4)) as 'phone', `institutes`.`email` as 'email', `institutes`.`pincode` as 'pincode', `institutes`.`city` as 'city', `institutes`.`state` as 'state', `institutes`.`ownerName` as 'ownerName', `institutes`.`ownerPhone` as 'ownerPhone', `institutes`.`ownerEmail` as 'ownerEmail', `institutes`.`adminName` as 'adminName', `institutes`.`adminPhone` as 'adminPhone', `institutes`.`adminEmail` as 'adminEmail', `institutes`.`subjects` as 'subjects'",
			'modules' => "`modules`.`moduleId` as 'moduleId', IF(    CHAR_LENGTH(`institutes1`.`instituteCode`) || CHAR_LENGTH(`institutes1`.`instituteName`), CONCAT_WS('',   `institutes1`.`instituteCode`, '-', `institutes1`.`instituteName`), '') as 'instituteNumber', IF(    CHAR_LENGTH(`courses1`.`courseCode`) || CHAR_LENGTH(`courses1`.`courseName`), CONCAT_WS('',   `courses1`.`courseCode`, '-', `courses1`.`courseName`), '') as 'courseId', IF(    CHAR_LENGTH(`assessments1`.`assessmentName`), CONCAT_WS('',   `assessments1`.`assessmentName`), '') as 'assessmentId', `modules`.`moduleName` as 'moduleName', `modules`.`link` as 'link', `modules`.`description` as 'description', `modules`.`file` as 'file'",
			'questions' => "`questions`.`id` as 'id', IF(    CHAR_LENGTH(`courses1`.`courseCode`) || CHAR_LENGTH(`courses1`.`courseName`), CONCAT_WS('',   `courses1`.`courseCode`, '-', `courses1`.`courseName`), '') as 'courseId', IF(    CHAR_LENGTH(`modules1`.`moduleName`), CONCAT_WS('',   `modules1`.`moduleName`), '') as 'moduleId', IF(    CHAR_LENGTH(`assessments1`.`assessmentName`), CONCAT_WS('',   `assessments1`.`assessmentName`), '') as 'assessmentId', `questions`.`question` as 'question', `questions`.`option1` as 'option1', `questions`.`option2` as 'option2', `questions`.`option3` as 'option3', `questions`.`option4` as 'option4', `questions`.`answer` as 'answer'",
			'students' => "`students`.`id` as 'id', IF(    CHAR_LENGTH(`institutes1`.`instituteCode`) || CHAR_LENGTH(`institutes1`.`instituteName`), CONCAT_WS('',   `institutes1`.`instituteCode`, '-', `institutes1`.`instituteName`), '') as 'instituteNumber', `students`.`username` as 'username', `students`.`password` as 'password', `students`.`firstname` as 'firstname', `students`.`middlename` as 'middlename', `students`.`lastname` as 'lastname', `students`.`gender` as 'gender', `students`.`email` as 'email', CONCAT_WS('-', LEFT(`students`.`phone`,3), MID(`students`.`phone`,4,3), RIGHT(`students`.`phone`,4)) as 'phone', if(`students`.`dob`,date_format(`students`.`dob`,'%d/%m/%Y'),'') as 'dob', if(`students`.`signupDate`,date_format(`students`.`signupDate`,'%d/%m/%Y %h:%i %p'),'') as 'signupDate', `students`.`city` as 'city', `students`.`state` as 'state'",
			'subjects' => "`subjects`.`subjectid` as 'subjectid', `subjects`.`subjectName` as 'subjectName'",
			'teachers' => "`teachers`.`id` as 'id', `teachers`.`firstname` as 'firstname', `teachers`.`middlename` as 'middlename', `teachers`.`lastname` as 'lastname', IF(    CHAR_LENGTH(`institutes1`.`instituteCode`) || CHAR_LENGTH(`institutes1`.`instituteName`), CONCAT_WS('',   `institutes1`.`instituteCode`, '-', `institutes1`.`instituteName`), '') as 'instituteNumber', CONCAT_WS('-', LEFT(`teachers`.`phone`,3), MID(`teachers`.`phone`,4,3), RIGHT(`teachers`.`phone`,4)) as 'phone', `teachers`.`email` as 'email', `teachers`.`pincode` as 'pincode', `teachers`.`city` as 'city', `teachers`.`state` as 'state', IF(    CHAR_LENGTH(`subjects1`.`subjectName`), CONCAT_WS('',   `subjects1`.`subjectName`), '') as 'subjects'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false) {
		$sql_from = array(
			'assessments' => "`assessments` LEFT JOIN `institutes` as institutes1 ON `institutes1`.`instituteNumber`=`assessments`.`instituteNumber` LEFT JOIN `courses` as courses1 ON `courses1`.`courseId`=`assessments`.`courseId` ",
			'courses' => "`courses` LEFT JOIN `institutes` as institutes1 ON `institutes1`.`instituteNumber`=`courses`.`instituteNumber` LEFT JOIN `teachers` as teachers1 ON `teachers1`.`id`=`courses`.`teacher` LEFT JOIN `subjects` as subjects1 ON `subjects1`.`subjectid`=`courses`.`subjects` ",
			'institutes' => "`institutes` ",
			'modules' => "`modules` LEFT JOIN `institutes` as institutes1 ON `institutes1`.`instituteNumber`=`modules`.`instituteNumber` LEFT JOIN `courses` as courses1 ON `courses1`.`courseId`=`modules`.`courseId` LEFT JOIN `assessments` as assessments1 ON `assessments1`.`assessmentId`=`modules`.`assessmentId` ",
			'questions' => "`questions` LEFT JOIN `courses` as courses1 ON `courses1`.`courseId`=`questions`.`courseId` LEFT JOIN `modules` as modules1 ON `modules1`.`moduleId`=`questions`.`moduleId` LEFT JOIN `assessments` as assessments1 ON `assessments1`.`assessmentId`=`questions`.`assessmentId` ",
			'students' => "`students` LEFT JOIN `institutes` as institutes1 ON `institutes1`.`instituteNumber`=`students`.`instituteNumber` ",
			'subjects' => "`subjects` ",
			'teachers' => "`teachers` LEFT JOIN `institutes` as institutes1 ON `institutes1`.`instituteNumber`=`teachers`.`instituteNumber` LEFT JOIN `subjects` as subjects1 ON `subjects1`.`subjectid`=`teachers`.`subjects` "
		);

		$pkey = array(
			'assessments' => 'assessmentId',
			'courses' => 'courseId',
			'institutes' => 'instituteNumber',
			'modules' => 'moduleId',
			'questions' => 'id',
			'students' => 'id',
			'subjects' => 'subjectid',
	...
	function htmlUserBar(){
		...
		?> <style>
			.bg-green	{
				background: linear-gradient(to right, rgba(112,204,145,1) 0%,rgba(111,187,248,1) 100%);
			}
		</style>
		<nav class="navbar bg-green navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<!-- application title is obtained from the name besides the yellow database icon in studemy, use underscores for spaces -->
				<a style=" color:black;"class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home " style="margin-right:10px;"></i> STUDEMY</p></a>
			</div>
				...
				<?php if(!$_GET['signIn'] && !$_GET['loginFailed']){ ?>
					...
					<?php }else{ ?>
						<ul class="nav navbar-nav navbar-right hidden-xs" style="min-width: 330px;">
							<a class="btn navbar-btn btn-danger" href="<?php echo PREPEND_PATH; ?>index.php?signOut=1"><i class="glyphicon glyphicon-log-out"></i> <?php echo $Translation['sign out']; ?></a>
							<p class="navbar-text"style=" color:black;">
								<?php echo $Translation['signed as']; ?> <strong><a style=" color:black;" href="<?php echo PREPEND_PATH; ?>membership_profile.php" class="navbar-link"><?php echo getLoggedMemberID(); ?></a></strong>
							</p>
						</ul>
	...
	function NavMenus($options = array()){
		...
		if(is_array($arrTables)){
			foreach($arrTables as $tn => $tc){
				...
				$tChkHL = array_search($tn, array('questions'));
		...
		for($i = 0; $i < count($menu); $i++){
			$menu_wrapper .= <<<EOT
				<li class="dropdown">
					<a style=" color:black;" href="#" class="dropdown-toggle" data-toggle="dropdown">{$table_group_name[$i]} <b class="caret"></b></a>
	...
	function quick_search_html($search_term, $label, $separate_dv = true){
		...
		<div class="input-group" id="quick-search">
			...
			<span class="input-group-btn">
				<button name="Search_x" value="1" id="Search" type="submit" onClick="{$reset_selection}" class="btn btn-success" title="{$safe_label}"><i class="glyphicon glyphicon-search"></i></button>
				<button name="ClearQuickSearch" value="1" id="ClearQuickSearch" type="submit" onClick="\$j('#SearchString').val(''); {$reset_selection}" class="btn btn-danger" title="{$safe_clear_label}"><i class="glyphicon glyphicon-remove-circle"></i></button>
			</span>
		</div>
EOT;
		return $html;
