	...
	$lookups = array(   
		'assessments' => array(   
			'instituteNumber' => array(
				...
				'parent_pk_field' => 'instituteNumber',
				'parent_caption' => 'IF(CHAR_LENGTH(`institutes`.`instituteCode`) || CHAR_LENGTH(`institutes`.`instituteName`), CONCAT_WS(\'\', `institutes`.`instituteCode`, \'-\', `institutes`.`instituteName`), \'\')',
				'parent_from' => '`institutes` ',
				'filterers' => array(),
				'custom_query' => 'SELECT `institutes`.`instituteNumber`, IF(CHAR_LENGTH(`institutes`.`instituteCode`) || CHAR_LENGTH(`institutes`.`instituteName`), CONCAT_WS(\'\', `institutes`.`instituteCode`, \'-\', `institutes`.`instituteName`), \'\') FROM `institutes` ORDER BY 2',
			...
			'courseId' => array(
				...
				'parent_pk_field' => 'courseId',
				'parent_caption' => 'IF(CHAR_LENGTH(`courses`.`courseCode`) || CHAR_LENGTH(`courses`.`courseName`), CONCAT_WS(\'\', `courses`.`courseCode`, \'-\', `courses`.`courseName`), \'\')',
				'parent_from' => '`courses` LEFT JOIN `institutes` as institutes1 ON `institutes1`.`instituteNumber`=`courses`.`instituteNumber` LEFT JOIN `teachers` as teachers1 ON `teachers1`.`id`=`courses`.`teacher` LEFT JOIN `subjects` as subjects1 ON `subjects1`.`subjectid`=`courses`.`subjects` ',
				'filterers' => array('instituteNumber' => 'instituteNumber'),
				'custom_query' => 'SELECT `courses`.`courseId`, IF(CHAR_LENGTH(`courses`.`courseCode`) || CHAR_LENGTH(`courses`.`courseName`), CONCAT_WS(\'\', `courses`.`courseCode`, \'-\', `courses`.`courseName`), \'\') FROM `courses` LEFT JOIN `institutes` as institutes1 ON `institutes1`.`instituteNumber`=`courses`.`instituteNumber` LEFT JOIN `teachers` as teachers1 ON `teachers1`.`id`=`courses`.`teacher` LEFT JOIN `subjects` as subjects1 ON `subjects1`.`subjectid`=`courses`.`subjects` ORDER BY 2',
				'inherit_permissions' => true,
				'list_type' => 0,
				'not_null' => false
			)
		...
		'courses' => array(   
			...
			'teacher' => array(
				...
				'parent_pk_field' => 'id',
				'parent_caption' => '`teachers`.`firstname`',
				'parent_from' => '`teachers` LEFT JOIN `institutes` as institutes1 ON `institutes1`.`instituteNumber`=`teachers`.`instituteNumber` LEFT JOIN `subjects` as subjects1 ON `subjects1`.`subjectid`=`teachers`.`subjects` ',
				'filterers' => array('instituteNumber' => 'instituteNumber'),
				'custom_query' => 'SELECT `teachers`.`id`, `teachers`.`firstname` FROM `teachers` LEFT JOIN `institutes` as institutes1 ON `institutes1`.`instituteNumber`=`teachers`.`instituteNumber` LEFT JOIN `subjects` as subjects1 ON `subjects1`.`subjectid`=`teachers`.`subjects` ORDER BY 2',
				'inherit_permissions' => true,
				'list_type' => 0,
				'not_null' => false
			),
			'subjects' => array(
				...
				'parent_pk_field' => 'subjectid',
				'parent_caption' => '`subjects`.`subjectName`',
				'parent_from' => '`subjects` ',
				'filterers' => array(),
				'custom_query' => 'SELECT `subjects`.`subjectid`, `subjects`.`subjectName` FROM `subjects` ORDER BY 2',
		...
		'modules' => array(   
			...
			'assessmentId' => array(
				...
				'parent_pk_field' => 'assessmentId',
				'parent_caption' => '`assessments`.`assessmentName`',
				'parent_from' => '`assessments` LEFT JOIN `institutes` as institutes1 ON `institutes1`.`instituteNumber`=`assessments`.`instituteNumber` LEFT JOIN `courses` as courses1 ON `courses1`.`courseId`=`assessments`.`courseId` ',
				'filterers' => array('instituteNumber' => 'instituteNumber', 'courseId' => 'courseId'),
				'custom_query' => 'SELECT `assessments`.`assessmentId`, `assessments`.`assessmentName` FROM `assessments` LEFT JOIN `institutes` as institutes1 ON `institutes1`.`instituteNumber`=`assessments`.`instituteNumber` LEFT JOIN `courses` as courses1 ON `courses1`.`courseId`=`assessments`.`courseId` ORDER BY 2',
				'inherit_permissions' => true,
				'list_type' => 0,
				'not_null' => false
			)
		...
		'questions' => array(   
			...
			'moduleId' => array(
				...
				'parent_pk_field' => 'moduleId',
				'parent_caption' => '`modules`.`moduleName`',
				'parent_from' => '`modules` LEFT JOIN `institutes` as institutes1 ON `institutes1`.`instituteNumber`=`modules`.`instituteNumber` LEFT JOIN `courses` as courses1 ON `courses1`.`courseId`=`modules`.`courseId` LEFT JOIN `assessments` as assessments1 ON `assessments1`.`assessmentId`=`modules`.`assessmentId` ',
				'filterers' => array('courseId' => 'courseId'),
				'custom_query' => 'SELECT `modules`.`moduleId`, `modules`.`moduleName` FROM `modules` LEFT JOIN `institutes` as institutes1 ON `institutes1`.`instituteNumber`=`modules`.`instituteNumber` LEFT JOIN `courses` as courses1 ON `courses1`.`courseId`=`modules`.`courseId` LEFT JOIN `assessments` as assessments1 ON `assessments1`.`assessmentId`=`modules`.`assessmentId` ORDER BY 2',
				'inherit_permissions' => true,
				'list_type' => 0,
				'not_null' => false
			),
