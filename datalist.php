...
class DataList{
	...
	// handle user commands ...
		...
		elseif($delete_x != ''){
			...
			if($delete_res){
				...
				if($Embedded){
					$this->HTML .= '<script>$j(function(){ setTimeout(function(){ studemy.closeParentModal(); }, 2000); })</script>';
				}
			}
		}
		...
		if(!$this->HideTableView && !($dvprint_x && $this->AllowSelection && $SelectedID) && !$PrintDV){
			...
			if($Print_x == ''){
				...
				if($this->AllowPrinting){
					$buttons_all .= '<button onClick="document.myform.NoDV.value=1; ' . $resetSelection . ' return true;" type="submit" name="Print_x" id="Print" value="1" class="btn btn-primary"><i class="glyphicon glyphicon-print"></i> ' . $Translation['Print Preview'] . '</button>';
					$buttonsCount++;
				}
				...
				$buttons_all .= '<button class="btn btn-danger" type="submit"><i class="glyphicon glyphicon-remove-circle"></i> ' . $Translation['Cancel Printing'] . '</button>';
			}
	...
	function tv_tools(){
		...
		<div class="pull-right flip btn-group vspacer-md hspacer-md tv-tools">
			<button title="<?php echo html_attr($Translation['previous column']); ?>" type="button" class="btn btn-default tv-scroll" onclick="studemy.TVScroll().less()"><i class="glyphicon glyphicon-step-backward"></i></button>
			<button title="<?php echo html_attr($Translation['next column']); ?>" type="button" class="btn btn-default tv-scroll" onclick="studemy.TVScroll().more()"><i class="glyphicon glyphicon-step-forward"></i></button>
		</div>
		...
		<script>
			$j(function(){
				...
				var show_column = function(col_class, show){
					...
					studemy.TVScroll().reset();

					col_cookie(col_class, show);
				}

				/* initiate TVScroll */
				studemy.TVScroll().less();
